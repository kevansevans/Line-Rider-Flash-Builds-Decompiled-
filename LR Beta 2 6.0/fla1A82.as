...
function onKeyUp()
{
    ...
    if (Key.getCode() == 9 && simulation == false && panel.fileDialog._visible == false && panel.confirmDialog._visible == false)
    {
        ...
        {
            if (lines.length != tempLines)
            {
                tempLines = lines.length;
                getBounds();
            } // end if
        ...
        {
            var _loc7 = tla._xmouse;
            var _loc6 = tla._ymouse;
            var _loc5 = _xmouse;
            var _loc4 = _ymouse;
            canvasX = tla._x = -(_loc7 * tempZoom * 0.010000 - _loc5);
            canvasY = tla._y = -(_loc6 * tempZoom * 0.010000 - _loc4);
...
function showAll()
{
    var _loc4 = maxX - minX;
    var _loc3 = maxY - minY;
    var _loc2 = Stage.width / _loc4;
    var _loc1 = Stage.height / _loc3;
    if (_loc2 > _loc1)
    {
        bufScale = tla._xscale = tla._yscale = _loc1 * 100;
        canvasX = tla._x = -minX * _loc1 + (Stage.width - _loc4 * _loc1) * 0.500000;
    ...
    else
    {
        ...
        canvasY = tla._y = -minY * _loc2 + (Stage.height - _loc3 * _loc2) * 0.500000;
    } // end else if
} // End of the function
function onKeyDown()
{
                ...
                onMouseMove = function ()
                {
                    if (active && active2 && cursor._currentframe != 2)
                    {
                        ...
                        if (prevLine != undefined && pencilMode != 2)
                        {
                            ...
                            tempLine.valid = tempLine.dst > MIN_LENGTH * _loc2 && checks[0] && checks[2] > MIN_ANGLE;
                        }
                        else
                        {
                            tempLine.valid = tempLine.dst > MIN_LENGTH * _loc2;
                        } // end else if
        ...
        if (active2)
        {
            ...
            else
            {
                ...
                onMouseMove = function ()
                {
                    ...
                    {
                        checks = checkAngles(tempLine, prevLine);
                        tempLine.valid = tempLine.dst > MIN_LENGTH * _loc1 && checks[0];
                    }
                    else
                    {
                        tempLine.valid = tempLine.dst > MIN_LENGTH * _loc1;
                    } // end else if
...
function render()
{
    ...
    {
        _loc3 = tla._xscale;
        _loc1 = Math.min(_loc3 * 1.100000, 400);
        world._xscale = world._yscale = tla._xscale = tla._yscale = _loc1;
        world._x = tla._x = Stage.width * 0.500000 + (tla._x - Stage.width * 0.500000) * (_loc1 / _loc3);
        world._y = tla._y = Stage.height * 0.500000 + (tla._y - Stage.height * 0.500000) * (_loc1 / _loc3);
    } // end if
...
panel.fileDialog.saveButton.onRelease = function ()
{
    if (Lines > 0)
    {
        ...
        if (_loc4 != undefined)
        {
            var _loc12 = savedTracks.data.trackList.splice(_loc4, 1);
        } // end if
        savedTracks.data.trackList.addItemAt(0, {label: _loc5, data: _loc6, level: totLines, startLine: [tla.startPoint._x, tla.startPoint._y]});
