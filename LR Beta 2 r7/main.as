...
function onKeyUp()
{
   ...
   var _loc4_;
   var _loc1_;
   if(panel.fileDialog._visible == false && panel.confirmDialog._visible == false && helpscreen._visible == false)
   {
      if(simulation == false)
      {
         ...
         if(Key.getCode() == 70 && tla.flag._visible == true)
         {
         ...
         if(panel.swatches._visible == true)
         {
            if(Key.getCode() == 49)
            {
   ...
   if(Key.getCode() == 72 && simulation == false && panel.fileDialog._visible == false && panel.confirmDialog._visible == false)
   {
      ...
      else
      {
         ...
         panel.lineB.onRollOut = panel.swatches.blue.onRollOut = panel.swatches.red.onRollOut = panel.swatches.green.onRollOut = panel.zoomB.onRollOut = panel.eraserB.onRollOut = panel.flagB.onRollOut = panel.handB.onRollOut = panel.pencilB.onRollOut = panel.playB.onRollOut = panel.stopB.onRollOut = panel.saveB.onRollOut = panel.trashB.onRollOut = panel.lineB.onReleaseOutside = panel.swatches.blue.onReleaseOutside = panel.swatches.red.onReleaseOutside = panel.swatches.green.onReleaseOutside = panel.zoomB
         .onReleaseOutside = panel.eraserB.onReleaseOutside = panel.flagB.onReleaseOutside = panel.handB.onReleaseOutside = panel.pencilB.onReleaseOutside = panel.playB.onReleaseOutside = panel.stopB.onReleaseOutside = panel.saveB.onReleaseOutside = panel.trashB.onReleaseOutside = function()
         {
            active2 = true;
            cursorImage(cursorMode);
         };
...
function erase(x, y)
{
   ...
   while(_loc14_ < 2)
   {
      ...
      if(grid[_loc6_] != undefined)
      {
         ...
         while(_loc7_ < 2)
         {
            _loc4_ = "y" + (_loc15_.y + _loc7_);
            if(grid[_loc6_][_loc4_] != undefined)
            {
               for(var _loc16_ in grid[_loc6_][_loc4_].storage2)
               {
                  _loc1_ = grid[_loc6_][_loc4_].storage2[_loc16_];
                  _loc3_ = x - _loc1_.x1;
                  _loc2_ = y - _loc1_.y1;
                  _loc9_ = Math.sqrt(Math.pow(_loc3_,2) + Math.pow(_loc2_,2));
                  _loc10_ = Math.sqrt(Math.pow(x - _loc1_.x2,2) + Math.pow(y - _loc1_.y2,2));
                  _loc11_ = Math.abs(_loc1_.nx * _loc3_ + _loc1_.ny * _loc2_);
                  _loc5_ = (_loc3_ * _loc1_.dx + _loc2_ * _loc1_.dy) * _loc1_.invSqrDis;
                  if(_loc9_ < ERASER_SIZE * _loc8_ || _loc10_ < ERASER_SIZE * _loc8_ || _loc11_ < ERASER_SIZE * _loc8_ && _loc5_ >= 0 && _loc5_ <= 1)
                  {
                     if(_loc1_.type == pencilMode)
                     {
                        removeLine(_loc1_);
                     }
                  }
...
function addLine(line)
{
   ...
   if(steucMode)
   {
      steuc.text = "B:" + lcBlue + " R:" + lcRed + " G:" + lcGreen;
   }
   else
   {
      steuc.text = Lines + " lines";
                  ...
                  if(tempLine.valid && _xmouse < Stage.width && _xmouse > 0 && _ymouse < Stage.height && _ymouse > 0)
                  {
                     ...
                     switch(pencilMode)
                     {
                        ...
                        case 2:
                           ...
                           break;
                        case 3:
                           _loc1_ = new FloorForce(tempLine.x1,tempLine.y1,tempLine.x2,tempLine.y2,tempLine.inv);
                     }
                     addLine(_loc1_);
                     if(pencilMode != 2 || pencilMode != 3)
                     {
   ...
   onEnterFrame = function()
   {
      ...
      if(!slowMotion || bur - pre > 600)
      {
         ...
         if(saveNow)
         {
            ...
            faceState = world.sankac.face._currentframe;
            ttl = timeToLook;
            frameNumber = absoluteFrameCounter;
         }
         frameCounter++;
         absoluteFrameCounter++;
         if(framez)
         {
            steuc.text = absoluteFrameCounter;
         }
      }
   };
   onMouseMove = function()
...
function renderLine(line)
{
   ...
   switch(line.type)
   {
      ...
      case 2:
         ...
         break;
      case 3:
         _loc6_ = line.dst >= 25 ? line.dx * line.invDst * 25 : Math.abs(line.dx);
         _loc5_ = line.dst >= 25 ? line.dy * line.invDst * 25 : Math.abs(line.dy);
         _loc2_.lineStyle(5 * SCALEDOWN,11382189,100,true,"normal","none","miter");
         _loc2_.moveTo(line.x2,line.y2);
         _loc2_.beginFill(11382189,40);
         if(line.dst > 25)
         {
            _loc2_.lineTo(line.x2 + (line.nx * 20 - _loc6_),line.y2 + (line.ny * 20 - _loc5_));
         }
         _loc2_.lineTo(line.x1 + line.nx * 20,line.y1 + line.ny * 20);
         _loc2_.lineTo(line.x1 - line.nx * 20,line.y1 - line.ny * 20);
         if(line.dst > 25)
         {
            _loc2_.lineTo(line.x2 + ((- line.nx) * 20 - _loc6_),line.y2 + ((- line.ny) * 20 - _loc5_));
...
function render()
{
   ...
   if(!faceLooking)
   {
      if(Math.abs(dots[4].x - bufx) < 0.7 && Math.abs(dots[4].y - bufy) < 0.7)
      {
         timeToLook++;
      }
      else
...
function satisfyConstraints()
{
   ...
   while(_loc7_ < 4)
   {
      dots[_loc7_].crash = undefined;
      _loc7_ = _loc7_ + 1;
   }
   var _loc9_ = 0;
   while(_loc9_ < ITERATE)
   {
   ...
   while(_loc7_ < 4)
   {
      if(dots[_loc7_].crash != undefined && Math.abs(dots[_loc7_].dx) + Math.abs(dots[_loc7_].dy) > 2)
      {
         dots[_loc7_].crash.particle(dots[_loc7_]);
         dots[_loc7_].crash.particle(dots[_loc7_]);
      }
      _loc7_ = _loc7_ + 1;
   }
   ...
   dots2[1].x += Math.random() * 0.3 * (- Math.min(dots2[1].dx,125));
   dots2[1].y += Math.random() * 0.3 * (- Math.min(dots2[1].dy,125));
   ...
   switch(ver)
   {
      case "6":
         ...
         registerInGrid = function(line)
         {
            ...
            {
               _loc4_ = Math.min(_loc7_.y,_loc8_.y);
      ...
      case "6.1":
         ...
         registerInGrid = function(line)
         {
            ...
            while(true)
            {
               ...
               _loc4_ = line.dy <= 0 ? - (_loc1_.gy + 1) : GRIDSIZE - _loc1_.gy;
               if(line.dx == 0)
               {
                  _loc2_ += _loc4_;
               }
               ...
               {
                  _loc7_ = Math.round(_loc9_ * (_loc5_ + _loc6_) + _loc8_);
      ...
      case "6.2":
         ...
         registerInGrid = function(line)
         {
            ...
            while(true)
            {
               ...
               {
                  _loc6_ = _loc4_ + line.dy * difX * _loc9_;
...
mouseListener.onMouseWheel = function(delta)
{
   ...
   {
      _loc1_ = tla._xscale;
      _loc2_ = Math.min(Math.max(_loc1_ + _loc1_ * 0.04 * delta,5),600);
...
panel.lineB.onRollOver = panel.swatches.green.onRollOver = panel.swatches.red.onRollOver = panel.swatches.blue.onRollOver = panel.swatches.grey.onRollOver = panel.zoomB.onRollOver = panel.eraserB.onRollOver = panel.flagB.onRollOver = panel.handB.onRollOver = panel.pencilB.onRollOver = panel.playB.onRollOver = panel.stopB.onRollOver = panel.saveB.onRollOver = panel.trashB.onRollOver = function()
{
   active2 = false;
   cursor._visible = false;
   Mouse.show();
};
panel.lineB.onRollOut = panel.swatches.blue.onRollOut = panel.swatches.red.onRollOut = panel.swatches.green.onRollOut = panel.swatches.grey.onRollOut = panel.zoomB.onRollOut = panel.eraserB.onRollOut = panel.flagB.onRollOut = panel.handB.onRollOut = panel.pencilB.onRollOut = panel.playB.onRollOut = panel.stopB.onRollOut = panel.saveB.onRollOut = panel.trashB.onRollOut = panel.lineB.onReleaseOutside = panel.swatches.blue.onReleaseOutside = panel.swatches.red.onReleaseOutside = panel.swatches.green
.onReleaseOutside = panel.zoomB.onReleaseOutside = panel.eraserB.onReleaseOutside = panel.flagB.onReleaseOutside = panel.handB.onReleaseOutside = panel.pencilB.onReleaseOutside = panel.playB.onReleaseOutside = panel.stopB.onReleaseOutside = panel.saveB.onReleaseOutside = panel.trashB.onReleaseOutside = function()
{
   active2 = true;
   cursorImage(cursorMode);
};
...
{
   var _loc1_;
   if(Key.isDown(68) && helpscreen._visible == false)
   {
      if(Key.isDown(16))
      {
         _loc1_ = getFirstLine();
...
panel.swatches.grey.onPress = function()
{
   pencilMode = 3;
   cursor.pencil.gotoAndStop(3);
...
panel.playB.onPress = function()
{
   ...
   if(Lines > 0)
   {
      ...
      if(saved && !Key.isDown(65))
      {
         restore();
         timeToLook = ttl;
         absoluteFrameCounter = frameNumber;
         if(!riding)
         {
            BindStick.crash = false;
            world.sanke.gotoAndStop(sledState);
         }
         if(faceState > 1 && faceState < 19)
         {
            world.sankac.face.gotoAndPlay(faceState);
            faceLooking = true;
         }
         else
         {
...
panel.saveB.onPress = function()
{
   ...
   panel.fileDialog.saveButton.onRelease = function()
   {
      ...
      if(Lines > 0)
      {
         ...
         _loc5_ = removeSpaces(panel.fileDialog.name.text);
         _loc2_ = 0;
         ...
         if(_loc4_ != undefined)
         {
            _loc13_ = savedTracks.data.trackList.splice(_loc4_,1);
         }
               ...
               while(_loc3_ < loadPerFrame && segmentsRemainingToLoad > 0)
               {
                  ...
                  switch(_loc1_[9])
                  {
                     ...
                     case 2:
                        ...
                        break;
                     case 3:
                        _loc2_ = new FloorForce(_loc1_[0],_loc1_[1],_loc1_[2],_loc1_[3],Boolean(_loc1_[5]),_loc1_[4]);
                  }
                  Lines++;
                  switch(_loc2_.type)
                  {
